...
func QueryCategoryHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		l := category.NewQueryCategoryLogic(r.Context(), svcCtx)
		resp, err := l.QueryCategory(&req)
		if err != nil {
			if cerror.CheckIsDefinedCategoryError(err) {
				response.ErrWithMessage(r.Context(), w, status.Convert(err).Message())
				return
			} else {
				l.Logger.Error("查询分类失败: ", err)
				response.ErrWithMessage(r.Context(), w, "查询分类失败")
				return
			}
		}
